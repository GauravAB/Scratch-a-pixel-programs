...
int main(int argc, char** argv)
{
	...
	//get the bounding box for object
	...
	//find the absolute max and min to set the canvas coordinates
	...
	//Set openGL orthographic matrix
	...
	//loop over all vertices in the scene
	for (uint32_t i = 0; i < numVertices; ++i)
	{
		...
		if (projectedVert.x < -imageAspectRatio || projectedVert.x > imageAspectRatio
			|| projectedVert.y < -1 || projectedVert.y > 1) continue;
