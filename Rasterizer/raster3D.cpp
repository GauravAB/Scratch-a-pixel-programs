...
)
{
	...
	//canvas = tan(theta) * distancetoImagePlane
	//canvasHeight/2
	...
	//adjust canvas ratio according to the flim gate and fit Mode
	right *= xscale;
	top *= yscale;
...
)
{
	...
	//project to screenspace ( a canvas on imaginary plane )
	Vec2f vertexScreen;
	vertexScreen.x = near * vertexCamera.x / -vertexCamera.z;
	vertexScreen.y = near * vertexCamera.y / -vertexCamera.z;

	//now convert to NDC space (in range [-1,1] GPU convention)
	Vec2f vertexNDC;
	...
	//keep camera coordinate z as it is just positive value
...
//just a cross product AC and BC
...
{	
	...
	//iterate all triangles from the scene
	for (uint32_t i = 0; i < ntris; ++i)
	{
		...
		//need to convert in raster space 
		...
		//clamp bounding box inside image resolution
		...
		for (uint32_t y = y0; y <= y1; ++y)
		{
			for (uint32_t x = x0; x <= x1; ++x)
			{
				...
				//calculate barycentric coordinates if the pixel overlaps the triangle
				if (w0 >= 0 && w1 >= 0 && w2 >= 0)
				{
					w0 /= area;
					w1 /= area;
					...
					//z coordinate of pixel sample in camera space
					float z = 1 / oneOverZ;
					...
					if (z < depthBuffer[y * imageWidth + x])
					{
						...
						//coordinates of interpolated point in camera space
	...
	std::cerr << "Wall passed time: " << passedTime << std::endl;


	std::ofstream ofs;
	ofs.open("./output1.ppm", std::ios::binary);
	ofs << "P6\n";
