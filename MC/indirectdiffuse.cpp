        ...
        switch (isect.hitObject->type) {
            ...
            case kDiffuse:
            {
                ...
                for (uint32_t n = 0; n < N; ++n) {
                    ...
                    indirectLigthing = indirectLigthing + ( r1 * castRay(hitPoint + sampleWorld * options.bias,
                        sampleWorld, objects, lights, options, depth + 1) * (1 / pdf) );
                }
                // divide by N
                indirectLigthing /= (float)N;
#endif

                hitColor = ( (directLighting * (1 / M_PI)) + 2 * indirectLigthing) * isect.hitObject->albedo;
                break;
            }
            default:
                break;
