...
// false otherwise.
// Computing pixel coordinates in raster space using world coordinate vertices of the object
...
)
{
	...
	pNDC.y = (pScreen.y + r) / (2 * r);

	printf("NDC coordinates are : %f %f\n", pNDC.x, pNDC.y);
...
//Settings of physical camera ( focal_length (mm)(near plane here) and flim frame (inches)(canvas))
// clipping planes
// resolution frame ( image dimentions)
// fit flim mode ( overscan or fit )

float focalLength = 35; // mm
// 35 mm full Aperture in inches
float flimApertureWidth = 0.825;
float flimApertureHeight = 0.446;
...
FitResolutionGate fitFlim = kOverscan;

int main(int argc, char** argv)
{
	//The canvas is positioned by choice at the near clipping plane
	float flimAspectRatio = flimApertureWidth / flimApertureHeight;
	float deviceAspectRatio = imageWidth / (float)imageHeight;

	float top = (((flimApertureHeight * inchToMm) / 2) / focalLength) * nearClippingPlane;
	float right = (((flimApertureWidth * inchToMm) / 2) / focalLength) * nearClippingPlane;
	...
	{
	case kFill:
		if (flimAspectRatio > deviceAspectRatio)
		{
			xscale = flimAspectRatio / deviceAspectRatio;
		}
		else
		{
			yscale = flimAspectRatio / deviceAspectRatio;
		}
		break;
	case kOverscan:
		if (flimAspectRatio > deviceAspectRatio)
		...
		{
			xscale = deviceAspectRatio / flimAspectRatio;
		}
		break;
	default:
		break;
	...
	printf("Flim Aspect Ratio : %f\n Device Aspect Ratio: %f \n", flimAspectRatio, deviceAspectRatio);
	printf("Angle of view: %f (deg)\n", 2 * atan( ((flimApertureWidth * inchToMm) / 2) / focalLength ) * 180 / M_PI);
	...
	for (uint32_t i = 0; i < numTris; ++i)
	{
		...
		ofs << "<line x1=\"" << v0Raster.x << "\" y1=\"" << v0Raster.y
			...
			<< "\" style=\"stroke:rgb(" << val << ",0,0);stroke-width:l\" />\n";
