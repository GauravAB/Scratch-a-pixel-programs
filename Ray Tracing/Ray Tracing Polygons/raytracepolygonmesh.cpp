...
public:
	...
	{
		bool isect = false;
		uint32_t numTris = vertexIndex.size() / 3;

		for (uint32_t i = 0; i < numTris; ++i)
		{
			const Vec3f& v0 = verts[vertexIndex[i*3]];
			const Vec3f& v1 = verts[vertexIndex[i*3 + 1]];
			const Vec3f& v2 = verts[vertexIndex[i*3 + 2]];
		...
		Vec2f& hitTextureCoordinates) const
	{
		//face normal for flat shading
 		const Vec3f& v0 = verts[vertexIndex[triIndex * 3]];
		const Vec3f& v1 = verts[vertexIndex[triIndex * 3 + 1]];
		const Vec3f& v2 = verts[vertexIndex[triIndex * 3 + 2]];
		hitNormal = (v2 - v0).crossProduct(v1 - v0);
		hitNormal.normalize();
	
		//texture coordinates
		const Vec2f & st0 = uvs[textureIndex[triIndex * 3]];
		const Vec2f & st1 = uvs[textureIndex[triIndex * 3 + 1]];
		const Vec2f & st2 = uvs[textureIndex[triIndex * 3 + 2]];
		hitTextureCoordinates = (1.0f - uv.x - uv.y) * st0 + uv.x * st1 + uv.y * st2;

		/*
		//vertex normals for smooth shading
		Vec3f n0 = normals[normalIndex[triIndex * 3]];
		n0.normalize();
		Vec3f  n1 = normals[normalIndex[triIndex * 3 + 1]];
		n1.normalize();
		Vec3f  n2 = normals[normalIndex[triIndex * 3 + 2]];
		n2.normalize();
		hitNormal = (1 - (uv.x + uv.y) ) * n0 + uv.x * n1 + uv.y * n2;
		*/
	...
	std::vector<uint8_t> vertexIndex;
	std::vector<uint8_t> textureIndex;
	std::vector<uint8_t> normalIndex;
	uint32_t numTris;
};
...
bool ObjMesh::loadObjFile(const char* file)
{
	...
	if (ifs.fail())
	{
		std::cerr << "ifs failed to open file " << file << std::endl;
		return false;
	}
	...
	while (!ifs.eof())
	{
		...
		else if (!line.compare(0, 2, "f "))
		{
			...
			if (3 != cnt)
			{
				std::cerr << "Obj file is supposed to be triangulated" << std::endl;
				ifs.close();
				return false;
			}
		}
	...
	numTris = vertexIndex.size() / 3;
	std::cerr << "# v#" << verts.size() << " f# " << vertexIndex.size() / 3 << " vt# " << uvs.size() << " vn# " << normals.size() << std::endl;
	std::cerr << "numTris: " << numTris << std::endl;
...
{	
	...
	sprintf(buff,"sp4.%04d.ppm",frame);
	std::ofstream ofs;
